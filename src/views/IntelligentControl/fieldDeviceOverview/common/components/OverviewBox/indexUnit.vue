<template>
  <div class="overview-box indexUnit">
    <el-form
      ref="overviewBoxForm"
      :model="data"
      :rules="rules"
    ...
    >
      ...
      <div v-if="fieldList.hasOwnProperty('body')" class="box-body" :class="childStatus.length > 4 ? 'box-body2' : ''">
...
export default {
  ...
  methods: {
    ...
    getBodyStatus() {
      this.childStatus = []
      for (const i of this.fieldList.body) {
        if (i.isShow) {
          this.childStatus.push(i.isShow)
        }
      }
    },
    radioEvent(event) {
