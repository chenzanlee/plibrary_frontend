          ...
          <el-tabs v-model="activeName" @tab-click="handleClick">
            ...
            <el-tab-pane label="设备控制" name="second">
              ...
              <div v-else>
                ...
                <equipment-control v-if="overview.isBoar === 1" id="control" v-hasAuth="'fieldDeviceOverview:controlSubDevice_MCU'" :unit-id="currentdeviceId" />
                <equipment-control-v2 v-else id="control" v-hasAuth="'fieldDeviceOverview:controlSubDevice_MCU'" :unit-id="currentdeviceId" />
              </div>
            </el-tab-pane>
            <!--  -->
            <el-tab-pane label="参数设置" name="third">
              <template v-if="overview.isBoar === 1">
                <!-- 参数设置 -->
                <parameter-setting
              ...
              <template v-else>
                <parameter-setting-v2
                  v-if="progress === 'parameterSetting'"
                  :unit-id="currentdeviceId"
                />
                <multi-unit-v2 v-if="progress === 'multiUnit'" :unit-id="currentdeviceId" />
                <multi-unit-setting-v2 v-if="progress === 'multiUnitSetting'" :key="time" :unit-id="currentdeviceId" />
                <multi-function-setting-v2 v-if="progress === 'multiFunctionSetting'" :key="multiFunctionTime" :unit-id="currentdeviceId" />
              </template>
            </el-tab-pane>
          </el-tabs>
        </div>
        <div>
          <unit-monitoring v-if="activeName === 'first' && overview.isBoar === 1" :info="fieldDeviceOverviewDetail" :unit-id="currentdeviceId" @childrenInfo="alarmInfo" />
          <unit-monitoring-v2 v-if="activeName === 'first' && overview.isBoar !== 1" :info="fieldDeviceOverviewDetail" :unit-id="currentdeviceId" @childrenInfo="alarmInfo" />
        </div>
      </div>
    </div>
  </div>
...
<script>
import unitMonitoringV2 from '@/views/IntelligentControl/fieldDeviceOverview/page/subpage/unitMonitoring'
import equipmentControlV2 from '@/views/IntelligentControl/fieldDeviceOverview/page/subpage/equipmentControl'
import parameterSettingV2 from '@/views/IntelligentControl/fieldDeviceOverview/page/subpage/parameterSetting'
import multiUnitV2 from '@/views/IntelligentControl/fieldDeviceOverview/page/subpage/subordinate/multiUnitSetUp'
import multiUnitSettingV2 from '@/views/IntelligentControl/fieldDeviceOverview/page/subpage/multiUnitSetting'
import multiFunctionSettingV2 from '@/views/IntelligentControl/fieldDeviceOverview/page/subpage/multiFunctionSetting'

import unitMonitoring from '@/views/IntelligentControl/fieldDeviceOverview/page/subpageV4/unitMonitoring'
import equipmentControl from '@/views/IntelligentControl/fieldDeviceOverview/page/subpageV4/equipmentControl'
import parameterSetting from '@/views/IntelligentControl/fieldDeviceOverview/page/subpageV4/parameterSetting'
import multiFunctionSetting from '@/views/IntelligentControl/fieldDeviceOverview/page/subpageV4/multiFunctionSetting'
    ...
    switchUnit(type) {
      ...
      this.unitTimer = setTimeout(() => {
        if (this.unitList && this.currentdeviceId) {
          ...
          // console.log('1111111111', this.overview)
        }
      }, 300)
    },
