            ...
            <div class="container-carrier3-col-main-div" :class="(i.type !== '1' && i.type !== '2' ? 'container-carrier3-col-main-div2' : '')">
              <div class="container-carrier3-col-main-div-body">
                <div class="container-carrier3-col-main-div-body-div1">
                  <img v-if="i.type === '5'" src="@/assets/image/unitStatus/equipment-yacha.png">
                  <img v-if="i.type === '6'" src="@/assets/image/unitStatus/equipment-fengliang.png">
                  <img v-if="i.type !== '5' && i.type !== '6'" src="@/assets/image/unitStatus/equipment-wendu.png">
                  {{ i.type === '5' ? '压差1' : i.type === '6' ? '通风量' : '温度' }}
                </div>
                <div class="container-carrier3-col-main-div-body-div2">
                  ...
                  <i>{{ i.type === '5' ? 'pa' : i.type === '6' ? 'm³/h' : '°C' }}</i>
                </div>
              </div>
            </div>
            <div class="container-carrier3-col-main-div" :class="(i.type !== '1' && i.type !== '2' ? 'container-carrier3-col-main-div2' : '')">
              <div class="container-carrier3-col-main-div-body">
                <div class="container-carrier3-col-main-div-body-div1">
                  <img v-if="i.type === '5'" src="@/assets/image/unitStatus/equipment-yacha.png">
                  <img v-if="i.type === '6'" src="@/assets/image/unitStatus/equipment-fengsu.png">
                  <img v-if="i.type !== '5' && i.type !== '6'" src="@/assets/image/unitStatus/equipment-shidu.png">
                  {{ i.type === '5' ? '压差2' : i.type === '6' ? '风速' : '湿度' }}
                </div>
                <div class="container-carrier3-col-main-div-body-div2">
                  <!-- <span>{{ i.humidity === -100 ? '-' : i.humidity }}</span> -->
                  <span>{{ ( i.humidity >= -99 && i.humidity !== null && i.humidity !== undefined) ? i.humidity : '-' }}</span>
                  <i>{{ i.type === '5' ? 'pa' : i.type === '6' ? 'm/s' : '%' }}</i>
                </div>
              </div>
            </div>
          ...
          <template v-for="(i, index) in equipmentStatusList" v-show="equipmentStatusList.length > 0">
            ...
            >
              <template v-if="i.tips !== '4'">
                ...
                <span v-if="i.tips && i.tips === '喷淋'" class="iconfont container-carrier4-body-div-topr">{{ i.an }}</span>
                <!-- 设备 -->
                <el-tooltip v-if="i.name.indexOf('喷淋') > -1" placement="top">
                  <div slot="content" class="showerTooltip">
                    <div><span>开启时长</span>: {{ i.open || '-' }}秒</div>
                    <div><span>关闭时长</span>: {{ i.close || '-' }}秒</div>
                    <div><span>工作模式</span>: {{ i.mode === 0 ? '停止' : i.mode === 1 ? '增湿' : i.mode === 2 ? '降温' : i.mode === 3 ? '除尘' : '-' }}</div>
                    <div><span>本次总用水量</span>: {{ i.allWater || '-' }} L</div>
                    <div><span>本次头均用水量</span>: {{ i.avgWater || '-' }} mL</div>
                  </div>
                  <div
        ...
        <div v-loading="loadingEchart" class="container-carrier container-carrier5">
          <div class="container-carrier5-head">
            <div class="container-carrier5-head-l">
              ...
              >通风量</div>
              <div
                v-if="info.overview.deviceVersionId === '6aKGZaUNLp9poYSNu1PhnU'"
                class="container-carrier-btn"
                :class="chartStatus === 'fanGear' ? 'container-carrier-btn-active' : ''"
                @click="switchLine('fanGear') "
              >风机档位</div>
              <div
                v-if="info.overview.deviceVersionId === '6aKGZaUNLp9poYSNu1PhnU'"
                class="container-carrier-btn"
...
export default {
  ...
  beforeRouteLeave(to, form, next) {
    ...
    clearInterval(this.showerTimer1)
    clearInterval(this.showerTimer2)
  ...
  methods: {
    ...
    handleData() {
      ...
      // 头均通风量
      let avg_wind = '-'
      // console.log(33333333, this.lineEchartsList[this.lineEchartsList.length - 1].actualWind, this.unitInfo.data[2].number, this.unitInfo.data)
    ...
    //       }
    //       // console.log('获取的天气this.weatherList1', this.weatherList, arr)
    //       arr[0].newDay = '今天'
    ...
    timeDifference() {
      const t = new Date(this.uploadTime).getTime()
      // const t = new Date('2021-01-09 17:56:00').getTime()
      const newT = new Date().getTime()
      const differenceT = (newT - t) < 0 ? 0 : Math.floor((newT - t) / 1000)
      return differenceT
    },
    setShow() {
      clearInterval(this.showerTimer1)
      clearInterval(this.showerTimer2)
      for (const i of this.equipmentStatusList) {
        if (i.name === '喷淋1') {
          this.showerTime(i, 1)
        }
        if (i.name === '喷淋2') {
    ...
    showerTime(i, n) {
      let num = i.time - this.timeDifference()
      if (num < 0) { return }
      this[`showerTimer${n}`] = setInterval(() => {
        i.an = `时长：${num}s`
        if (num <= 0) {
          i.status = 0
          i.an = ''
          clearInterval(this[`showerTimer${n}`])
          this.getPigInfo()
        }
        num--
      }, 1000)
    ...
    handlePigInfo(res) {
      ...
      // 压差
      this.equipmentList[4].temperature = data && data.pressure1 || '-'
      this.equipmentList[4].humidity = data && data.pressure2 || '-'
      // 其他
      ...
      for (let i = 0; i < 10; i++) {
        ...
        if (data[n3] !== undefined) {
          const j3 = {
            ...
            status: data[n3],
            waring: (data[f3] > 0),
            close: i === 1 ? data.showerCloseDuration : data[`shower${i}CloseDuration`],
            open: i === 1 ? data.showerOpenDuration : data[`shower${i}OpenDuration`],
            time: i === 1 ? data.showerRemain : data[`shower${i}Remain`],
            mode: i === 1 ? data.showerMode : data[`shower${i}Mode`],
            allWater: i === 1 ? data.showerAllWater : data[`shower${i}AllWater`],
            avgWater: i === 1 ? data.showerAvgWater : data[`shower${i}AvgWater`],
            an: '',
            tips: '喷淋'
          }
          console.log(9999999999999, j3)
          arr.push(j3)
        }
      }
      // 暖灯
