<template>
  <div class="facility titleHighlight">
    <h3 class="short-title2">湿度</h3>
    <div v-if="show" class="flexbg">
      <div ref="myEchartTwo" class="chart" />
    </div>
    <div v-if="!show" class="img-box" style="position:relative; top: 50px">
...
export default {
  ...
  data() {
    return {
      ...
      option: {
        ...
        legend: {
          data: ['室内前湿度', '室内前湿度均值', '室内后湿度', '室内后湿度均值'],
          textStyle: {
            color: '#fff' // 这里用参数代替了
          }
        },
        ...
        tooltip: {
          ...
          formatter: (val) => {
            ...
            val.forEach((item) => {
              if (item.seriesName === '室内前湿度') {
                str += `<br/><span style="display: inline-block; width:80px; padding-left:24px;">前湿: </span>`
                str += '谷值: ' +
                ...
                str += `峰值: ` +
                            ...
                            '%</span>'
              } else if (item.seriesName === '室内后湿度') {
                str += `<br/><span style="display: inline-block; width:80px; padding-left:24px;">后湿: </span>`
                str += `谷值: ` +
                            `<span style="display: inline-block; width:100px;">` +
                            (item.value[1] && item.value[1].toFixed(1) || '--') +
                            `%</span>`
                str += `峰值: ` +
                            ...
                            (item.value[2] && item.value[2].toFixed(1) || '--') +
                            `%</span>`
              } else if (item.seriesName === '室内前湿度均值' || item.seriesName === '室内后湿度均值') {
                str += `均值: ` +
                            `<span style="display: inline-block;width:100px;">` +
  ...
  computed: {
    startZoom() {
      const now = this.$moment().subtract(1, 'days').format('YYYY-MM-DD HH:mm:ss')
      const before = [...this.mess.filter((item, index) => {
        const now_date = this.$moment(now)
        const date = this.$moment(item.uploadTime)
        if (date < now_date) {
          return item
        }
      })]
  ...
  'watch': {
    ...
    'allStyles.flag'(newValue, oldValue) {
      this.$refs.myEchartTwo && this.resetChart() // 自己绘制echarts的方法
    }
  },
  mounted() {
  },
    ...
    setEcharts(n) {
      ...
      n.forEach(item => {
        ...
        this.option.series[0].data.push([
          item.minHumiditieInner1 === -100 ? null : item.minHumiditieInner1,
          item.maxHumiditieInner1 === -100 ? null : item.maxHumiditieInner1,
          item.minHumiditieInner1 === -100 ? null : item.minHumiditieInner1,
          item.maxHumiditieInner1 === -100 ? null : item.maxHumiditieInner1
        ])
        this.option.series[1].data.push(item.meanHumiditieInner1 === -100 ? null : item.meanHumiditieInner1)
        this.option.series[2].data.push([
          item.minHumiditieInner2 === -100 ? null : item.minHumiditieInner2,
          item.maxHumiditieInner2 === -100 ? null : item.maxHumiditieInner2,
          item.minHumiditieInner2 === -100 ? null : item.minHumiditieInner2,
          item.maxHumiditieInner2 === -100 ? null : item.maxHumiditieInner2
        ])
        this.option.series[3].data.push(
          item.meanHumiditieInner2 === -100 ? null : item.meanHumiditieInner2
        )
      })
