  ...
  <div class="sortbg">
    ...
    <div v-if="this.listData.length" class="main-contain overfolw">
      ...
      <div v-for="(item,index) in listData" :key="index">
        <div :class="[ item.impRate < 50 ? 'list-container list-container-red' : 'list-container']">
          <div class="list-index">{{ index + 1 }}</div>
          <div class="list-name">{{ item.regionName }}</div>
          <div class="list-data">{{ item.impCount !== undefined ? item.impCount : 100 }}/{{ item.totalUnitCount !== undefined ? item.totalUnitCount : 100 }}</div>
          <div class="list-percentage">{{ item.impRate ? item.impRate : 0 }}%</div>
        </div>
      </div>
    </div>
