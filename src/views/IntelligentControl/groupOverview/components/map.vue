...
export default {
  name: 'Echarts',
  props: ['region', 'allList', 'area', 'field', 'getMapData', 'paramsJson', 'choose', 'mapData'],
  data() {
    return {
      pageElement: [], // 页面权限数组
      mapdata: {},
  ...
  methods: {
    ...
    convertPointData(data, values) {
      ...
      arr.map((item) => {
        obj = {
          ...
          'data': this.initRegionData(item.properties.fieldId, values, 3),
    ...
    async drawChart(name, distributeStats) {
      ...
      if (name === '全国') {
        ...
        for (var i = 0; i < data.features.length; i++) {
          d.push({
            name: data.features[i].properties.name,
            value: _this.isCompanyName(data.features[i].properties.name) ? _this.getWarn(data.features[i].properties.areaId, statRegionData, 2) : -1,
            data: _this.initRegionData(data.features[i].properties.areaId, statRegionData, 2),
            regionId: data.features[i].properties.regionId || '',
            areaId: data.features[i].properties.areaId || '',
            parentName: data.features[i].properties.parentName
          })
        }
        _this.renderMap(name, d, [], option, this.myMapChart)
        ...
        option.geo = { // 散点地图相关配置
          ...
          itemStyle: { // 地图区域的配置
            normal: {
              areaColor: '#004ED2', // 绘制的每一个轮廓的背景颜色
              borderColor: '#81CDF6'
            },
            emphasis: {
              areaColor: '#004ED2'// 鼠标移到区域时的背景颜色
            }
          }
        }
    ...
    goToReviewPage(val) { // 跳转到下一级
      ...
      getUnitEvc({
        regionId: dropRegions.value
      })
      sessionStorage.setItem('unitDataOverviewRegions', JSON.stringify(dropRegions))
      this.$router.push({
        path: 'areaView'
      })
    },
    ...
    formatterToolTip(params) {
      const powerRegion = this.region
      const powerArea = this.area
      const powerField = this.field
