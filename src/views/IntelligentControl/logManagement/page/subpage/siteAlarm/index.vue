<template>
  <div class="home siteAlarm">
    ...
    >
      ...
      <span slot="right" :authCode="'alarmLog:export'" @click="exportData()">
...
export default {
            ...
            btnLists: [
              {
                innerText: '查询',
                className: 'btn-blue',
                authCode: 'alarmLog:listAlarmLogs',
                handleClick: () => {
  ...
  methods: {
    ...
    getfieldWranningList() {
      ...
      const element = getConfigElement('alarmLog')
      console.log(element)
      this.$httpRequest({
        type: element ? element.listAlarmLogs.method : '',
        uri: element ? element.listAlarmLogs.uri : '',
        payload: obj,
        callback: (res) => {
      ...
      // fieldWranningList(obj).then((res) => {
      //   this.isLoading = false
      //   this.buttonLoading = false
      //   // this.tableData = res.data.rows;
      //   const arr = res.data.rows
    ...
    exportData() {
      ...
      console.log(this.search, 333333)
      this.search.pageIndex = this.pagination.pageIndex
      this.search.pageSize = this.pagination.limit
      if (this.search.regionId === '') {
        this.$message({
      ...
      derive('/api' + url, this.handleObj(this.search))
    },
    // 分页器页码
