  ...
  <div>
    <div class="multiUnit-row default">
      ...
      <div class="multiUnit-padding multiUnit-select">
        ...
        <el-radio-group v-model="deviceVersionId" style="margin-left:15px;">
          <el-radio v-for="(item) in deviceVersionList" :key="item.deviceVersionId" :label="item.deviceVersionId">{{ item.deviceVersionName }}</el-radio>
        </el-radio-group>
      </div>
      <div v-if="devicesFieldList.length>0" v-loading="devicesFieldListLoading" class="multiUnit-padding multiUnit-Fchecked">
        <el-checkbox
      ...
      <div v-else v-loading="devicesFieldListLoading" class="multiUnit-padding">
        暂无单元
      </div>
    </div>
    <div class="multiUnitFst-foot">
      <span>点击下一步进入“参数控制”页面</span>
      <button
  ...
  methods: {
    ...
    getDeviceVersion(data) {
      ...
      data.forEach((item, index) => {
        if (!obj[item.deviceVersionId]) {
          obj[item.deviceVersionId] = item.versionName
          arr.push({
    ...
    pointIt(i, type) {
      if (type === 'all') {
        ...
        if (i.statusOnline === 1) {
          ...
          for (const j of this.devicesFieldList) {
            if (j.unit === i.unit && j.unitId === i.unitId) {
              j.selectS = j.selectS === 0 ? 1 : 0
            }
    ...
    goTo() {
      if (this.selectunitList) {
        ...
        sessionStorage.setItem('setMultiUnit', JSON.stringify({
        ...
        this.$store.dispatch('IntelligentControl/setMultiUnit', {
          deviceNames: json.deviceNames,
          eviceNames: json.deviceNames,
          unitNames: json.name,
          fieldId: this.overviewDetail.fieldId,
          fieldName: this.overviewDetail.fieldName,
          deviceVersionId: json.deviceVersionId
        })
        this.overviewDetails.progress = 'multiUnitSetting'
      } else {
    ...
    goBack() {
      ...
      this.overviewDetails.progress = 'parameterSetting'
    },
    // 处理单元信息
    ...
    async devicesForField() {
      this.devicesFieldListLoading = true
      await getDevicesByFieldForBuilding({
        fieldId: this.overviewDetail.fieldId,
        block: this.blockValue
      }).then((res) => {
        this.devicesFieldListLoading = false
        if (res && res.status === 200 && res.rel) {
          this.devicesFieldDate = res.data.deviceList
          this.deviceVersionList = this.getDeviceVersion(res.data.deviceList)
          this.deviceVersionId = this.deviceVersionList[0].deviceVersionId
