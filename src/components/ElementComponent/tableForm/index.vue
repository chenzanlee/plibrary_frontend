    ...
    >
      ...
      <div class="chaxun">
        <template v-for="(item, index) in formConfig.formItemList">
          <div :key="index" class="chaxun-cli">
            <template>
              <div>
                <template v-if="['text', 'textarea', 'number', 'email'].indexOf(item.type) !== -1">
                  <el-form-item :rules="item.rules ? item.rules : '' " :prop="item.prop ? item.prop : ''">
                    <el-input v-model="value[item.name]" class="noShadow" :type="item.type" :placeholder="item.placeholder" />
                  </el-form-item>
                </template>
                <!-- select下拉框 -->
                <template v-if="item.type === 'select'">
                  <el-form-item :rules="item.rules ? item.rules : '' " :prop="item.prop ? item.prop : ''">
                    <el-select v-model="value[item.name]" :placeholder="item.placeholder" class="shadow">
                      <el-option
                ...
                <template v-if="item.type === 'selectany'">
                  <el-form-item :rules="item.rules ? item.rules : '' " :prop="item.prop ? item.prop : ''">
                    <el-select v-model="value[item.name]" :placeholder="item.placeholder" class="shadow">
                      <el-option
                        v-for="(j, k) in item.optList"
                        :key="k"
                        :value="j[item.customCode]"
                      />
                    </el-select>
                  </el-form-item>
                </template>
                ...
                <template v-if="item.type == 'cascader'">
                  <el-form-item>
                    <el-cascader ref="cascader" v-model="value[item.name]" clearable :options="item.optList" class="shadow" :placeholder="item.placeholder">
      ...
      <div class="searchBtn">
        <el-button-group>
          <template v-if="formConfig.vShow" class="ele">
            <el-button
              v-for="(item, j) in formConfig.operate"
              :key="j"
              :class="item.class"
              :size="item.size"
          ...
          <template v-if="!formConfig.vShow">
            <button
...
export default {
  ...
  created() {
    for (var i = 0; i < this.formConfig.formItemList.length; i++) {
      this.value[this.formConfig.formItemList[i].name]
    }
